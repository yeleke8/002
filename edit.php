...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $data['contacts'] = $contacts; // Для шаблона (если будет ошибка)
    ...
    if (empty($errors)) {
        try {
            ...
            $sql = "UPDATE post SET title=?, description=?, address=?, photo=?, contacts=?, status=?, updated_at=NOW() WHERE post_id=?";
            $pdo->prepare($sql)->execute([
                $data['title'], $data['description'], $data['address'], $photoPath, 
                json_encode($contacts, JSON_UNESCAPED_UNICODE), $newStatus, $post_id
            ]);
