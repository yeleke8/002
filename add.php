...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Собираем данные формы обратно в data, чтобы они не пропадали при ошибке
    $data = [
    ...
    if (empty($errors)) {
        ...
        try {
            $sql = "INSERT INTO post (title, slug, description, address, photo, contacts, owner_id, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, 0, NOW())";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                $data['title'], $slug, $data['description'], $data['address'], $photoPath, 
                json_encode($data['contacts'], JSON_UNESCAPED_UNICODE), $_SESSION['user_id']
            ]);
